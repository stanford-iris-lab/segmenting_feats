...
seeds=("123" "124" "125")
embedding_names=("clip_vit16")
load_paths=("clip_vit16")
camera_names=("left_cap2" "right_cap2")
run_id=0 # change this to rerun experiment with the same parameters
        ...
        for camera_name in ${camera_names[@]} ; do
            for embedding_name in ${embedding_names[@]} ; do
                for load_path in ${load_paths[@]} ; do
                    sbatch run.sh ${env_name} ${seed} ${camera_name} ${embedding_name} ${load_path} ${run_id}
                done
            done
        done
    done
